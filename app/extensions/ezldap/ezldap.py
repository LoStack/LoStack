...
class LDAPManager:    
    ...
    def setup_ldap_structure(self):
        ...
        self.create_admin_user_and_group()
    ...
    def create_admin_user_and_group(
        ...
        user_dn = self._dn_user(username)
        group_dn = self._dn_group(self.admins_group)
        ...
        try:
            self._search(group_dn, ldap.SCOPE_BASE, "(objectClass=*)", ["cn"])
            self.logger.info(f"Admin group {self.admins_group} already exists")
            group_exists = True
        except ldap.NO_SUCH_OBJECT:
            group_exists = False
        
        # If both exist, just ensure membership
        if admin_exists and group_exists:
            return self.add_user_to_group(username, self.admins_group)
        ...
        if not group_exists:
            ...
            self.connection.add_s(group_dn, modlist.addModlist(prepared_group_attrs))
            self.logger.info(f"Successfully created admin group: {self.admins_group}")
        elif not admin_exists:
            # Group exists but user is new, add user to group
            self.add_user_to_group(username, self.admins_group)
